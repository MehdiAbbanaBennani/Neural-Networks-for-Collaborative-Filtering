...
class Dataset(object):
    def __init__(self, dataset):
        ...
        self.nb_users = dataset[0].shape[0]
        self.nb_movies = dataset[0].shape[1]
        self.nb_elements = dataset[0].nnz

        self.index_completed = np.zeros(7, dtype=int)
        self.last_batch_size = np.zeros(7, dtype=int)
        self.epoch_completed = np.ones(7, dtype=int) * -1
